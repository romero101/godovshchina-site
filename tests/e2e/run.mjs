...
for (const env of ENVS) {
  ...
  if (RELEASE && ["3.1", "3.4", "3.5", "4.1", "4.2", "4.3", "4.4", "4.6", "4.7", "4.8", "4.10"].some(on)) await withPage(env, { id: "3.x" }, async (page, ctx) => {
    ...
    if (["3.1", "3.4", "3.5"].some(on)) {
      ...
      if (on("3.1")) rec(E, "3.1", !!popup && /polis812\.ru\/mortgage/.test(u) && /partnerId=212866/.test(u) && /click_id=gdv-/.test(u) && /whiteLabel=true&partnerParams=true/.test(u) && /bank_id=1&debt=2500000&object_type=flat&sex=male&dob=\d\d\.\d\d\.1986/.test(q) && stayed, `новая вкладка: ${!!popup}; ${q || u.slice(0, 80)}; click_id ${/click_id=gdv-/.test(u)}, whiteLabel ${/whiteLabel=true&partnerParams=true/.test(u)}; текущая осталась на сайте: ${stayed}`);
      if (popup) await popup.close();
      if (on("3.5")) { const goals = await page.evaluate(() => (window.__ym || []).filter(a => a[0] === "reachGoal").map(a => a[1])); rec(E, "3.5", goals.filter(g => g === "partner_click").length === 1 && goals.filter(g => g === "partner_open").length === 1, `reachGoal: ${goals.join(", ") || "нет"}`); }
    ...
    if (["4.4", "4.7", "4.8", "4.10", "4.6"].some(on)) {
      ...
      if (on("4.4")) { const [p44] = await Promise.all([ctx.waitForEvent("page", { timeout: 8000 }).catch(() => null), page.click("#p-submit")]); await page.waitForTimeout(400); const q = p44 ? decodeParams(p44.url()) : ""; rec(E, "4.4", !!p44 && /bank_id=1&debt=2500000&object_type=flat&sex=female&dob=15\.03\.1986/.test(q) && /whiteLabel=true&partnerParams=true/.test(p44.url()) && /click_id=gdv-/.test(p44.url()) && page.url().startsWith(SITE), `${q || "вкладка не открылась"}; whiteLabel+click_id ${p44 ? /whiteLabel=true&partnerParams=true/.test(p44.url()) && /click_id=gdv-/.test(p44.url()) : false}; текущая осталась на сайте: ${page.url().startsWith(SITE)}`); if (p44) await p44.close(); }
  ...
  if (RELEASE && on("9.1")) await withPage(env, { id: "9.1" }, async (page) => {
    const bad = [];
    for (const l of LANDINGS) { const t0 = Date.now(); const r = await page.request.get(SITE + l + "?utm_source=yandex&utm_medium=cpc&utm_campaign=search_banks&utm_content=1&utm_term=t"); const ms = Date.now() - t0; const html = r.ok() ? await r.text() : ""; if (!r.ok() || !/id="pform"/.test(html) || !/id="calc"/.test(html)) bad.push(`${l} ${r.status()} form:${/id="pform"/.test(html)} calc:${/id="calc"/.test(html)} ${ms}мс`); }
    rec(E, "9.1", bad.length === 0, bad.length ? bad.join("; ") : `${LANDINGS.length} посадочных: 200, калькулятор и форма партнёра есть (список фиксированный; после API Директа — из объявлений)`);
  });
